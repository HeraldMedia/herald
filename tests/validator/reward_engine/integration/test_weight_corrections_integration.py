...
from herald.validator.reward_engine.services.weight_corrections_service import WeightCorrectionsService
from herald.validator.reward_engine.services.reward_distribution_service import RewardDistributionService
from herald.validator.reward_engine.models.evaluation_result import (
    EvaluationResultCollection, EvaluationResult, AccountResult
)
from herald.validator.reward_engine.models.emission_target import EmissionTarget
from herald.validator.utils.weight_corrections_publisher import publish_weight_corrections
...
class TestWeightCorrectionsIntegration:
    ...
    def _create_test_evaluation_results(self):
        ...
        account_result = AccountResult(
            ...
            videos={
                "video_123": {
                    "details": {"heraldVideoId": "herald_video123"},
                    "brief_metrics": {
                        "brief_1": {
                            "base_score": 2.5,
                            "scaling_factor": 1800,
                ...
                "video_456": {
                    "details": {"heraldVideoId": "herald_video456"},
                    "brief_metrics": {
                        "brief_1": {
                            "base_score": 1.8,
                            "scaling_factor": 1800,
    ...
    def test_end_to_end_weight_corrections_pipeline(self):
        ...
        for correction in corrections:
            ...
            assert correction["content_id"].startswith("herald_")
    ...
    async def test_publishing_integration(self):
        ...
        with patch('herald.validator.utils.weight_corrections_publisher.UnifiedDataPublisher') as mock_publisher_class:
    ...
    def test_limited_content_handling(self):
        ...
        limited_corrections = [
            ...
            if c["content_id"] == "herald_video456"
        ]
    ...
    def test_platform_agnostic_design(self):
        ...
        # Verify all content IDs are platform-agnostic (herald_xxx format)
        for correction in corrections:
            content_id = correction["content_id"]
            assert content_id.startswith("herald_") or content_id.startswith("video_")  # Fallback case
