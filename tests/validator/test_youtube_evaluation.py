...
def test_check_channel_criteria():
    ...
    channel_data = {
        "heraldChannelId": "test_channel_1",
...
def test_acceptance_filter():
    ...
    channel_data = {
        "heraldChannelId": "test_channel_acceptance",
        "subCount": str(YT_MIN_SUBS + 1000),
        "channel_start": (datetime.now() - timedelta(days=YT_MIN_CHANNEL_AGE + 10)).strftime('%Y-%m-%dT%H:%M:%SZ')
    }
    channel_analytics = {
...
@pytest.mark.asyncio
@patch('herald.validator.platforms.youtube.api.clients.build')
@patch('herald.validator.platforms.youtube.evaluation.video.get_video_data_batch')
@patch('herald.validator.platforms.youtube.evaluation.video.get_video_analytics')
@patch('herald.validator.platforms.youtube.evaluation.video.get_video_transcript')
@patch('herald.validator.platforms.youtube.evaluation.video.state.is_video_already_scored')
@patch('herald.validator.platforms.youtube.evaluation.video.state.mark_video_as_scored')
@patch('herald.validator.platforms.youtube.evaluation.video.transcript.check_for_prompt_injection')
@patch('herald.validator.platforms.youtube.evaluation.video.brief_matching.evaluate_content_against_brief')
@patch('herald.validator.platforms.youtube.evaluation.video.brief_matching.ThreadPoolExecutor')
@patch('herald.validator.utils.config.DISABLE_LLM_CACHING', True)
