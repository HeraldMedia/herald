...
from herald.validator.utils.config import YOUTUBE_SEARCH_CACHE_EXPIRY, YT_MAX_VIDEOS
...
def get_video_data_batch(youtube_data_client, video_ids, discrete_mode=False):
    ...
    for i in range(0, len(video_ids), 50):
        ...
        for info in items:
            vid = info['id']
            bcid = ("herald_" + hashlib.sha256(vid.encode()).hexdigest()[:8]) if discrete_mode else vid
            result[vid] = {
                "videoId": vid,
                "heraldVideoId": bcid,
