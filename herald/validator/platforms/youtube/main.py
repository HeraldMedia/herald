...
from herald.validator.clients.llm_client import get_llm_request_count, reset_llm_request_count
from herald.validator.platforms.youtube.api import (
    get_channel_analytics,
    get_channel_data,
    initialize_youtube_clients,
)
from herald.validator.platforms.youtube.api.video import get_all_uploads
from herald.validator.platforms.youtube.evaluation import (
    calculate_video_score,
    vet_channel,
    vet_videos,
)
from herald.validator.platforms.youtube.utils import _format_error, state
from herald.validator.platforms.youtube.utils.historical_videos import (
...
from herald.validator.platforms.youtube.evaluation.curve_scoring import calculate_adjusted_curve_difference
from herald.validator.utils.token_pricing import get_herald_alpha_price, get_total_miner_emissions
...
def apply_video_limits(briefs, result):
    ...
    for brief in briefs:
            ...
            if video_data.get("matching_brief_ids") and brief_id in video_data["matching_brief_ids"]:
                ...
                if video_score > 0:
                    scored_videos.append({
                        "video_id": video_id,
                        "score": video_score,
                        "herald_video_id": video_data["details"].get("heraldVideoId", video_id)
                    })
        ...
        limited_video_ids = [v["herald_video_id"] for v in videos_to_limit]
        bt.logging.info(
...
def _calculate_per_video_metrics(
    ...
    try:
        ...
        alpha_price_usd = get_herald_alpha_price()
...
def update_video_score(video_id, youtube_analytics_client, video_matches, briefs, result, is_ypp_account, channel_analytics=None, min_stake=False):
    ...
    # Get herald video ID for logging (falls back to YouTube ID if not available)
    herald_video_id = result["videos"][video_id]["details"].get("heraldVideoId", video_id)
    ...
    video_score_result = calculate_video_score(
        ...
        herald_video_id=herald_video_id, min_stake=min_stake
    )
    ...
    for i, match in enumerate(video_matches.get(video_id, [])):
        if match:
            ...
            bt.logging.info(f"Brief: {brief_id}, Video: {result['videos'][video_id]['details']['heraldVideoId']}, "
