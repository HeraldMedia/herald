...
from herald.validator.clients.llm_client import evaluate_content_against_brief
from herald.validator.utils.error_handling import log_and_raise_processing_error
...
def evaluate_content_against_briefs(briefs, video_data, transcript, decision_details):
        ...
        for future in as_completed(future_to_brief):
            ...
            try:
                ...
                log_and_raise_processing_error(
                    ...
                    context={
                        "brief_id": brief["id"],
                        "video_id": video_data.get("heraldVideoId")
                    }
                )
    ...
    if selected_index is not None and selected_brief is not None:
        ...
        bt.logging.info(
            ...
            f"from {total_matches} matching briefs for video: {video_data.get('heraldVideoId')}"
        )
    ...
    for pp_index, pp_brief in pp_briefs:
        ...
        bt.logging.info(
            f"Product placement brief '{pp_brief['id']}' also matched for video: {video_data.get('heraldVideoId')}"
        )
    
    if selected_index is None and not pp_briefs:
        bt.logging.info(f"No briefs matched for video: {video_data.get('heraldVideoId')}")
