...
from herald.validator.platforms.youtube.api.video import get_video_analytics
from herald.validator.platforms.youtube.config import get_youtube_metrics
from herald.validator.utils.config import ECO_MODE, YT_REWARD_DELAY, YT_ROLLING_WINDOW
...
def calculate_video_score(video_id, youtube_analytics_client, video_publish_date, 
                         ...
                         herald_video_id: Optional[str] = None,
    ...
    Args:
        ...
        herald_video_id (Optional[str]): Herald video ID for logging (defaults to YouTube ID)
    ...
    # Use herald video ID for logging, fall back to YouTube video ID if not provided
    log_video_id = herald_video_id if herald_video_id is not None else video_id
