...
- Format: {"video_id": str, "herald_video_id": str, "channel_id": str, 
           "herald_channel_id": str, "date_first_matched": str, "brief_id": str}
"""
...
# Path: herald/validator/data/historical_videos.jsonl
...
def record_video_match(
    ...
    Args:
        ...
        herald_video_id: Herald video ID (possibly hashed for privacy)
        channel_id: YouTube channel ID (real ID, not hashed)
        herald_channel_id: Herald channel ID (possibly hashed for privacy)
    ...
    try:
        ...
        entry = {
            "video_id": video_id,
            "herald_video_id": herald_video_id,
            "channel_id": channel_id,
            "herald_channel_id": herald_channel_id,
...
def record_matching_video(
    ...
    try:
        channel_id = result["yt_account"]["details"].get("id")
        herald_channel_id = result["yt_account"]["details"].get("heraldChannelId")
        herald_video_id = video_data.get("heraldVideoId", video_id)
        
        if not (channel_id and herald_channel_id):
            return
