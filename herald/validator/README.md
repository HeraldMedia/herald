...
3. **Configure Environment**
   ```bash
   cp herald/validator/.env.example herald/validator/.env
   ```
   ...
   - `PM2_PROCESS_NAME`: (Optional) Custom name for PM2 process (default: herald_validator)
...
2. **Process Management with PM2**
   ...
   - Monitor status:
     ```bash
     pm2 show herald_validator
     ```
   - Restart if needed:
     ```bash
     pm2 restart herald_validator
     ```
