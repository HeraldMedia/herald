...
from herald.utils.misc import ttl_cache
...
def get_herald_alpha_price() -> float:
    """
    Get the current Herald price in USD from CoinGecko API.
    
    Returns:
        float: The current Herald price in USD
    ...
    response = requests.get(
        "https://api.coingecko.com/api/v3/simple/price?ids=herald&vs_currencies=usd", 
        timeout=10
    )
    ...
    if 'herald' not in data:
        raise KeyError("Herald data not found in API response")
    if 'usd' not in data['herald']:
        raise KeyError("USD price not found in Herald data")
    
    herald_usd_price = data['herald']['usd']
    
    if not isinstance(herald_usd_price, (int, float)) or herald_usd_price <= 0:
        raise ValueError(f"Invalid price value: {herald_usd_price}")
    
    return float(herald_usd_price)
